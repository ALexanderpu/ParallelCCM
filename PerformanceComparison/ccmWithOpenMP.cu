...
int main(int argc, char *argv[])
{
	...
	*/
    ...
    size_t E = 3;
    size_t tau = 1;
    size_t lib_size = min((size_t)500, num_vectors);
    ...
    // compute distance matrix using lag_vector  N*E
    cout << "calculate the distance matrix: " << endl;
    vector<vector<double> > distance_matrix(num_vectors, vector<double>(num_vectors, qnan));
    ...
    //todo:  then globally sort here and search
	...
	// final result
	vector<double > rhos;
    // run as many CPU threads as possible   cpu > gpu
	...
	for(size_t sample = 0; sample < num_samples; sample++){
		...
		for(size_t k = 0; k < which_pred.size(); k++){
			...
			//find nearest neighbors here: distance_matrix[cur_pred_index]
